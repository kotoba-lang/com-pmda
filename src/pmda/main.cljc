(ns pmda.main
  "Kotodama WASM entrypoint for the Pmda clean-room actor (L4) — Clojure port.
  ...
  facts (`pmda.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"pmda.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pmda-compat" :tier tier :entities entities} 200])
